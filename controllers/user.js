...
exports.postUser = function(req, res, next) {
  ...
  User.findById(req.body.id, function(err, user) {
    if (err) return next(err);
    user.judge = req.body.judge === 'on' ? true : false;
    user.save(function(err) {
      if (err) return next(err);
      req.flash('success', { msg: 'User updated.' });
      res.redirect('users');
    });
  });
};
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      ...
      var mailOptions = {
        to: user.email,
        from: mailer.config.from,
        subject: 'Reset your password on Hackathon board',
        text: 'You are receiving this email because you (or someone else) have requested the reset of the password for your account.\n\n' +
          'Please click on the following link, or paste this into your browser to complete the process:\n\n' +
          'http://' + req.headers.host + '/reset/' + token + '\n\n' +
          'If you did not request this, please ignore this email and your password will remain unchanged.\n'
      };
      console.log("Email sent from " + mailer.config.from + " to " + user.email + " for a password reset");
      transporter.sendMail(mailOptions, function(err) {
